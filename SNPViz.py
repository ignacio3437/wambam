...
k = 5 #number of K to run the Admixture analysis
bs = 10 # Number of bootstraps for RaxML analysis
taxon_cov = 0.90 # If locci has sample coverage below this number it will be thrown out.
threads = 7
...
outlierbutton = "outliersigmathresh: 7"### add     "outliersigmathresh: 7" if you want more outliers
##################################################################

#Set Global variables
datafile=pd.read_table("%s%s"%(pwd,datafile),dtype={'Sample': object,'COI_both': object})
baseo='%s_o'%(basename)
pcafile='%s%s.evec'%(pwd,basename)
admixoutdir=pwd+"admixture/"
admix_colorpalette_list = sns.color_palette(palette='Set1').as_hex()
colorpalette_list = sns.color_palette(palette='Dark2').as_hex()
...
def VcfToPlink(baseo):
    ...
    subprocess.check_output(shlex.split("plink2 --file %s --threads %d --indep-pairwise 50 10 0.1"%(baseo,threads)))
    subprocess.check_output(shlex.split("plink2 --file %s --threads %d --extract plink.prune.in --recode --freq --maf 0.05 --out %s"%(baseo,threads,basename)))
    return
...
def PCAer():
    ...
    lsqproject:    YES
    %s
        """%(basename,basename,basename,basename,basename,threads,outlierbutton))
    ...
    pcaaxis = ("PCA1___PercentVar="+pervar[0]+"_Pval="+pvals[0],"PCA2___PercentVar="+pervar[1]+"_Pval="+pvals[1])
...
def PCAPlotter(df,colorby,colorpalette):
    ...
    pcapplot = sns.lmplot(
        ...
        fit_reg=False,
        legend=True,
        scatter_kws={"linewidths":.1,'edgecolors':'black','s':25})
    pcapplot = (pcapplot.set_axis_labels(pcaaxis[0],pcaaxis[1]))
    # plt.legend(loc = 'best')
    plt.title('PCA plot',fontsize=8)
    pcapplot.savefig('%s%s_PCA_%s_%s%s'%(pwd,basename,colorby,df,imgformat),dpi=dpi)
    plt.clf()
    return
...
def MapSetUp(datatable):
    ...
    m = Basemap(epsg=3577,llcrnrlat=lllat,urcrnrlat=urlat,llcrnrlon=lllon,urcrnrlon=urlon)
    m.arcgisimage(service='ESRI_Imagery_World_2D', xpixels = xpix, verbose= True,dpi=dpi)
    return m
...
def AdmixtureRunner(k):
    ...
    for x in range(1,k+1):
        admixcommand=shlex.split("admixture -j%d -C=0.01 --cv %s.bed %d"%(threads,basename,x))
...
def AdmixtureSetUp(k):
    ...
    if len(outlierlist)>0:
        subprocess.check_output(shlex.split("plink2 --threads %d --file %s_o --make-bed --remove %s --out %s"%(threads,basename,outlierfile,basename)))
    else:
        subprocess.check_output(shlex.split("plink2 --threads %d --file %s_o --make-bed --out %s"%(threads,basename,basename)))
...
def draw_pie(ax,ratios, X, Y, size):
    ...
    for i, xyi in enumerate(xy):
        ax.scatter([X],[Y] , marker=(xyi,0), s=size, facecolor=admix_colorpalette_list[i],alpha=.6,edgecolors='none')
    return
...
def MapAdmix(lowestk):
    ...
    plt.savefig("%s%s_MAP_Admix%d%s"%(pwd,basename,lowestk,imgformat),dpi=dpi)
    plt.clf()
...
def Raxer(pwd,basename,bs):
    ...
    print "raxmlHPC-PTHREADS-SSE3 -T %d -f a -n %s -s %s.phy -x %d -N %d -m GTRCAT -p %d"%(threads,basename,basename,random.randint(0,999999),bs,random.randint(0,999999))
    print "raxmlHPC-PTHREADS-SSE3 -T %d -f x -n %s -s %s.phy -m GTRCAT -p %d"%(threads,basename,basename,random.randint(0,999999))
