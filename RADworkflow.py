...
def vcftoplink(pwd,basename,baseo,basep,taxon_cov):
    ...
    subprocess.check_output(shlex.split("vcftools --vcf %s.vcf --max-missing %.2f --out %s --remove-indv WAMM56247 --remove-indv WAMM49196 --plink"%(basename,taxon_cov,baseo)))
    ...
    subprocess.check_output(shlex.split("plink2 --file %s --threads 7 --extract plink.prune.in --recode --freq --out %s"%(baseo,basep)))
    return
...
def Rplot(pwd,basename,type,lowestk,taxon_cov):
    ...
    if "PCA" in type[1]:
        ...
        ro.r('map<-get_map(location=bounds,zoom=5,maptype="satellite");options(warn=0)')
...
def raxer(pwd,basename,bs):
    ...
    print "raxmlHPC-PTHREADS-SSE3 -T 8 -f x -n %s -s %s.phy -m GTRCAT -p %d"%(basename,basename,random.randint(0,999999))
...
def main():
    ...
    pwd="/Users/josec/Desktop/Planigale/So_outfiles/"
    basename="So"
    datafile="SoMeta.txt"
